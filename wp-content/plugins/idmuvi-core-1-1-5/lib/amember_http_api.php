...
if( !function_exists( 'idmuvi_core_license_page' ) ) {
	...
	function idmuvi_core_license_page() {
		...
		?>
		<div class="wrap">
			<h2><?php _e( 'Muvipro License Options', 'idmuvi-core' ); ?></h2>
					...
					<tbody>
						<tr valign="top">
							<th scope="row" valign="top">
								<?php _e( 'License Key', 'idmuvi-core' ); ?>
							</th>
							<td>
								...
								<label class="description" for="idmuvi_core_license_key"><?php _e( 'Enter your license key here', 'idmuvi-core' ); ?></label>
							</td>
						</tr>
						
						<tr valign="top">
							<th scope="row" valign="top">
								<?php _e( 'Activate License', 'idmuvi-core' ); ?>
							</th>
							
							<td>
								<?php if( !empty ( $status ) && $status == 'ok' ) { ?>
									<input type="submit" style="background: #dff0d8 !important;color: #3c763d !important;text-shadow: none !important;" class="button-secondary" name="" disabled value="<?php _e( 'License Active' ); ?>"/>
									<?php wp_nonce_field( 'idmuvi_core_license_nonce', 'idmuvi_core_license_nonce' ); ?>
									<input type="submit" class="button-secondary" name="idmuvi_core_license_deactivate" value="<?php _e( 'Deactivate License', 'idmuvi-core' ); ?>"/><br />
									<label class="description" for="idmuvi_core_license_key"><?php _e( 'Congratulations, your license is active.<br /> You can disable license for this domain by entering the license key to the form and clicking <strong>Deactivate License</strong>', 'idmuvi-core' ); ?></label>
									<?php idmuvi_core_check_license(); ?>
								<?php } else {
									wp_nonce_field( 'idmuvi_core_license_nonce', 'idmuvi_core_license_nonce' ); 
									?>
									<input type="submit" class="button-secondary" name="idmuvi_core_license_activate" value="<?php _e( 'Activate License', 'idmuvi-core' ); ?>"/>
...
if( !function_exists( 'idmuvi_core_register_option' ) ) {
	...
	function idmuvi_core_register_option() {
		...
		register_setting('idmuvi_core_license', 'idmuvi_core_license_key', 'idmuvi_core_sanitize_license' );
	}
}
...
if( !function_exists( 'idmuvi_core_sanitize_license' ) ) {
	...
	function idmuvi_core_sanitize_license( $new ) {
		$old = get_option( 'idmuvi_core_license_key' );
		if( $old && $old != $new ) {
			delete_option( 'idmuvi_core_license_status' ); // new license has been entered, so must reactivate
		}
		return $new;
	}
}
...
if( !function_exists( 'idmuvi_core_activate_license' ) ) {
	...
	function idmuvi_core_activate_license() {
		...
		if( isset( $_POST['idmuvi_core_license_activate'] ) ) {
			...
			$license = !empty( $_POST['idmuvi_core_license_key'] ) ? $_POST['idmuvi_core_license_key'] : '';
			$url = home_url();
				...
				if ( $license_data->code != 'ok' ) {
					switch( $license_data->code ) {
						...
						case 'license_expired' :
							$message = sprintf(
								__( 'Your license key expired on %s.', 'idmuvi-core' ),
						...
						case 'no_spare_activations' :
							$message = sprintf( __( 'No more activations allowed.', 'idmuvi-core' ) );
							break;
			...
			if( ! empty( $message ) && ( $license_data->scheme_id == '1' || $license_data->scheme_id == '2' || $license_data->scheme_id == '3' ) ){
				$base_url = admin_url( 'plugins.php?page=' . IDMUVI_PLUGIN_LICENSE_PAGE );
				$redirect = add_query_arg( array( 'idmuvi_core_activation' => 'false', 'message' => urlencode( $message ) ), $base_url );
				...
				update_option( 'idmuvi_core_license_key', $e_license ); 
				update_option( 'idmuvi_core_license_status', 'ok' );
				
				wp_redirect( admin_url( 'plugins.php?page=' . IDMUVI_PLUGIN_LICENSE_PAGE ) );
				exit();
			}
		}
	}
	...
	function idmuvi_core_deactivate_license() {
		...
		if( isset( $_POST['idmuvi_core_license_deactivate'] ) ) {
			...
			if( ! empty( $message ) && ( $license_data->scheme_id == '1' || $license_data->scheme_id == '2' || $license_data->scheme_id == '3' ) ){
				...
				update_option( 'idmuvi_core_license_status', '' );
...
if( !function_exists( 'idmuvi_core_check_license' ) ) {
	...
	function idmuvi_core_check_license() {
		...
		if ( false === get_transient( 'idmuvi-core-license-transient' ) ) {
			...
			$license = trim( get_option( 'idmuvi_core_license_key' ) );
		...
		if ( isset( $_GET['idmuvi_core_activation'] ) && ! empty( $_GET['message'] ) ) {
			...
			switch( $_GET['idmuvi_core_activation'] ) {
				...
				default:
					?>
					<div class="success">
						<p><?php echo _e('Success.', 'idmuvi-core'); ?></p>
					</div>
					<?php
					break;
