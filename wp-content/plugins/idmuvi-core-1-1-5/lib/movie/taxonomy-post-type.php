...
/**
 * Since wp v.4.8 same category not select, this function for change category hierarchial and same with tags.
 *
 * @since 1.0.7
 * @return void
 */
...
if( !function_exists( 'idmuvi_core_save_first_letter' ) ) {
	...
	function idmuvi_core_save_first_letter( $post_id ) {
		...
		$limitPostTypes = array( 'post', 'tv' );
		if ( isset( $_POST['post_type'] ) && ( !in_array($_POST['post_type'], $limitPostTypes) ) ) 
			return;
