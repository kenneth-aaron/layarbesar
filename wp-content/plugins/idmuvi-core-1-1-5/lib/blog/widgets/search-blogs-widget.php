...
class Idmuvi_Core_Search_Blogs extends WP_Widget {
    ...
    public function widget($args, $instance) {
		...
		if ( $title ) {
			...
			echo '<form role="search" method="get" class="search-form" action="' . esc_url( home_url( '/' ) ) . '">
					<label>
						<span class="screen-reader-text">' . __( 'Search for:', 'idmuvi-core' ) . '</span>
						<input type="search" class="search-field" placeholder="' . __( 'Search blogs &hellip;', 'idmuvi-core' ) . '" value="' . get_search_query() . '" name="s" />
					</label>
					<input type="hidden" name="post_type" value="blogs" />
					<input type="submit" class="search-submit" value="'. __( 'Search', 'idmuvi-core' ) .'" />
				</form>';
	...
	public function form( $instance ) {
		...
		?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'idmuvi-core'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></label></p>
		<?php
	}
}
