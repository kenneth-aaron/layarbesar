...
class Idmuvi_RecentBlogs_Wgt extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array( 'classname' => 'idmuvi-rp-widget', 'description' => __( 'Recent blogs with thumbnails widget.','idmuvi-core' ) );
	...
	public function admin_print_scripts() {
		...
		<script>
			function setSuggest_cat_recent_blog(id) {
				jQuery('#' + id).suggest("<?php echo admin_url('admin-ajax.php'); ?>?action=ajax-tag-search&tax=blog_category", {multiple:true, multipleSep: ","});
			}
			function setSuggest_tag_recent_blog(id) {
				jQuery('#' + id).suggest("<?php echo admin_url('admin-ajax.php'); ?>?action=ajax-tag-search&tax=blog_tag", {multiple:true, multipleSep: ","});
			}
		</script>
		<?php
	}
									...
									<a href="<?php the_permalink(); ?>" itemprop="url" title="<?php the_title_attribute( array( 'before' => __( 'Permalink to: ','idmuvi-core' ), 'after' => '' ) ); ?>"><?php 
										if ( $idmuv_show_thumb ) : 
											...
											if ( has_post_thumbnail() ) : 
												the_post_thumbnail( 'blog-small', array( 'itemprop'=>'image' ) );
												
											else :
												// do_action( 'funct', $size, $link, $classes = '', $echo = true );
												do_action( 'idmuvi_core_get_images', 'blog-small', false );
    ...
    public function form( $instance ) {
		...
		<p>
			...
			<input class="widefat" id="<?php echo $this->get_field_id( 'idmuv_category' ); ?>" name="<?php echo $this->get_field_name( 'idmuv_category' ); ?>" type="text" value="<?php echo $idmuv_category; ?>" onfocus ="setSuggest_cat_recent_blog('<?php echo $this->get_field_id( 'idmuv_category' ); ?>');" />
			<br />
		...
		<p>
			...
			<input class="widefat" id="<?php echo $this->get_field_id( 'idmuv_tag' ); ?>" name="<?php echo $this->get_field_name( 'idmuv_tag' ); ?>" type="text" value="<?php echo $idmuv_tag; ?>" onfocus ="setSuggest_tag_recent_blog('<?php echo $this->get_field_id( 'idmuv_tag' ); ?>');" />
			<br />
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('idmuv_show_meta'); ?>"><?php _e( 'Show blog meta?', 'idmuvi-core' ); ?></label>
		</p>
		<p>
	...
	private function get_tag_ID( $tag_name ) {
		// Try tag name first
		$tag = get_term_by( 'name', $tag_name, 'blog_tag' );
		if ( $tag ) {
			return $tag->term_id;
		} else {
			// if Tag name is not found, try tag slug
			$tag = get_term_by( 'slug', $tag_name, 'blog_tag' );
			if ( $tag ) {
				return $tag->term_id;
			}
			return 0;
	...
	private function get_cat_ID( $cat_name ) {
		// Try cat name first
		$cat = get_term_by( 'name', $cat_name, 'blog_category' );
		if ( $cat ) {
			return $cat->term_id;
		} else {
			// if cat name is not found, try cat slug
			$cat = get_term_by( 'slug', $cat_name, 'blog_category' );
			if ( $cat ) {
				return $cat->term_id;
			}
			return 0;
