<?php
/**
 * Widget API: Idmuvi_Aweber_form class
 *
...
/**
 * Add the Aweber widget.
 *
 * @since 1.0.0
 *
 * @see WP_Widget
 */
class Idmuvi_Aweber_form extends WP_Widget {
	/**
	 * Sets up a Aweber widget instance.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function __construct() {
		$widget_ops = array( 'classname' => 'idmuvi-form', 'description' => __( 'Add simple aweber form in your widget.','idmuvi-core' ) );
		parent::__construct( 'idmuvi-aweber', __( 'Aweber Form (Idmuvi)','idmuvi-core' ), $widget_ops );
	...
	/**
	 * Outputs the content for Aweber Form.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * @param array $instance Settings for Aweber Form.
	 */
    public function widget($args, $instance) {
		...
		// list ID option
        $idmuv_list_id        	 	= empty( $instance['idmuv_list_id'] ) ? '' : strip_tags( $instance['idmuv_list_id'] );
		// Redirect thankyou link
        $idmuv_thx_link       		= empty( $instance['idmuv_thx_link'] ) ? '' : strip_tags( $instance['idmuv_thx_link'] );
		// Redirect already on list
        $idmuv_onlist_redirect_url 	= empty( $instance['idmuv_onlist_redirect_url'] ) ? '' : strip_tags( $instance['idmuv_onlist_redirect_url'] );
		...
		?>
			...
			<div class="idmuvi-form-widget<?php if ( $idmuv_force_100 ) { echo ' force-100'; } ?>"<?php if ( $bgcolor ) { echo ' style="padding:20px;background-color:'.$bgcolor.'"'; } ?>>
				...
				<?php } ?>
				<form class="idmuvi-form-wrapper" id="<?php echo esc_attr( $idmuv_widget_ID ); ?>" method="post" name="<?php echo esc_attr( $idmuv_widget_ID ); ?>" action="//www.aweber.com/scripts/addlead.pl">
					
					<div style="display: none;">
						<input type="hidden" name="meta_split_id" value="" />
						<input type="hidden" name="listname" value="<?php echo esc_attr( $idmuv_list_id ); ?>" />
						<input type="hidden" name="redirect" value="<?php echo esc_url( $idmuv_thx_link ); ?>" id="" />
						<input type="hidden" name="meta_redirect_onlist" value="<?php echo esc_url( $idmuv_onlist_redirect_url ); ?>" />
					</div>
	...
	/**
	 * Handles updating settings for the current Aweber widget instance.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @param array $new_instance New settings for this instance as input by the user via
	 *                            Idmuvi_Aweber_form::form().
    ...
    public function update($new_instance, $old_instance) {
		...
		// list ID option
        $instance['idmuv_list_id']           			= strip_tags( $new_instance['idmuv_list_id'] );
		// Redirect thankyou link
        $instance['idmuv_thx_link']            			= strip_tags( $new_instance['idmuv_thx_link'] );
		// Redirect already on list
        $instance['idmuv_onlist_redirect_url'] 			= strip_tags( $new_instance['idmuv_onlist_redirect_url'] );
		// Activated name input
	...
	/**
	 * Outputs the settings form for the Aweber widget.
	 *
	 * @since 1.0.0
	 * @access public
	 *
    ...
    public function form( $instance ) {
		...
		// list ID option
        $idmuv_list_id           		= strip_tags( $instance['idmuv_list_id'] );
		// Redirect thankyou link
        $idmuv_thx_link             	= strip_tags( $instance['idmuv_thx_link'] );
		// Redirect already on list
        $idmuv_onlist_redirect_url 		= strip_tags( $instance['idmuv_onlist_redirect_url'] );
		// Activated name input
		...
		<p>
			<label for="<?php echo $this->get_field_id('idmuv_list_id'); ?>"><?php _e( 'Aweber List ID *(Required)','idmuvi-core' ); ?></label> 
			<input class="widefat" id="<?php echo $this->get_field_id('idmuv_list_id'); ?>" name="<?php echo $this->get_field_name('idmuv_list_id'); ?>" type="text" value="<?php echo esc_attr($idmuv_list_id); ?>" />
			<br />
            <small><?php _e( 'Input Unique List ID <a target="_blank" href="https://help.aweber.com/hc/en-us/articles/204028426-What-Is-The-Unique-List-ID-">Click Here to see where to find it.</a>','idmuvi-core' ); ?></small>
		</p>
		<p>
			...
			<br />
            <small><?php _e( 'Leave empty if you want to use Aweber\'s default thank you page.','idmuvi-core' ); ?></small>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('idmuv_onlist_redirect_url'); ?>"><?php _e( 'Redirect URL','idmuvi-core' ); ?></label> 
			<input class="widefat" id="<?php echo $this->get_field_id('idmuv_onlist_redirect_url'); ?>" name="<?php echo $this->get_field_name('idmuv_onlist_redirect_url'); ?>" type="text" value="<?php echo esc_url($idmuv_onlist_redirect_url); ?>" />
			<br />
            <small><?php _e( 'Redirect Already Subscribed Users to Following URL.','idmuvi-core' ); ?></small>
		</p>
		<p>
...
add_action( 'widgets_init', function() {
    register_widget( 'Idmuvi_Aweber_form' );
} );