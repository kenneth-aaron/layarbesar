<?php
/**
 * Widget API: Idmuvi_Mailchimp_form class
 *
...
/**
 * Add the Mailchimp widget.
 *
 * @since 1.0.0
 *
 * @see WP_Widget
 */
class Idmuvi_Mailchimp_form extends WP_Widget {
	/**
	 * Sets up a Mailchimp widget instance.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function __construct() {
		$widget_ops = array( 'classname' => 'idmuvi-form', 'description' => __( 'Add simple mailchimp form in your widget.','idmuvi-core' ) );
		parent::__construct( 'idmuvi-mailchimp', __( 'Mailchimp Form (Idmuvi)','idmuvi-core' ), $widget_ops );
    ...
    public function widget($args, $instance) {
		...
		?>
			...
			<div class="idmuvi-form-widget<?php if ( $idmuv_force_100 ) { echo ' force-100'; } ?>"<?php if ( $bgcolor ) { echo ' style="padding:20px;background-color:'.$bgcolor.'"'; } ?>>
				...
				<?php } ?>
				<form class="idmuvi-form-wrapper" id="<?php echo esc_attr( $idmuv_widget_ID ); ?>" method="post" name="<?php echo esc_attr( $idmuv_widget_ID ); ?>" action="<?php echo esc_url( $idmuv_action_url ); ?>">
					
					<input type="email" name="EMAIL" id="" class="idmuvi-form-email" placeholder="<?php echo esc_attr( $idmuv_placeholder_email ); ?>" required />
    ...
    public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('idmuv_action_url'); ?>"><?php _e( 'Mailchimp Action URL *(Required)','idmuvi-core' ); ?></label> 
			...
			<br />
            <small><?php _e( 'Mailchimp Action URL <a target="_blank" href="https://help.shopify.com/themes/customization/communication/get-a-mailchimp-form-action-url">Click Here to see where to find it.</a>','idmuvi-core' ); ?></small>
		</p>
		<p>
...
add_action( 'widgets_init', function() {
    register_widget( 'Idmuvi_Mailchimp_form' );
} );